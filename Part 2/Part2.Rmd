...
date:     "November 14, 2019<br><br> [https://jorgetendeiro.github.io/SHARE-UMCG-14-Nov-2019/Part2](https://jorgetendeiro.github.io/SHARE-UMCG-14-Nov-2019/Part2)"
header-includes:
  - \usepackage{amsmath}
output:   
...
<img src="qrplot.png" width="400" height="400" />
...
Say you have data $\mathcal{D}$, which you use to improve your understanding on the possible values of an unknown parameter $\theta$. For instance, $\theta$ can be the proportion of obese men in a particular population, where male obesity is operationalized as [percentage of body fat (PBF) above 25%](https://en.wikipedia.org/wiki/Body_fat_percentage){target="_blank"}. Data $\mathcal{D}$ are the PBF scores for a sample of men from that population.
...
Let's improve the prior distribution. Suppose we expected, a priori, that the proportion of obese men in the population would be [about 40%](https://en.wikipedia.org/wiki/Obesity_in_the_United_States){target="_blank"}. We can pick a prior which peakes around .4 and redo the computations:
...
text(x = .38, y = .8*ymax, paste0("p = ", p.Up30), col = "blue")
...
text(x = .38, y = .8*ymax, paste0("p = ", p.25to35), col = "blue")
...
Any person with the same set of 3 ingredients will update his/her knowledge in the same way. Also, persons with different priors use the same mechanism to update their belief (that is what I mean by *rational* update). "This" unique mechanism used to update our knowledge is given by the Bayes rule. The Bayes rule, by the way, is universal and accepted by frequentists and Bayesians alike (it is mathematically based on axioms). The Bayes rule is a mathematical necessity. Not accepting the Bayes rule is not accepting the basic axioms of probability.
...
I will illustrate how to perform Bayesian estimation with a simple example. To make the contrast with the frequentist analysis clearer, I will fit the same model in the classic way too. This way, we can better compare the two paradigms. R will be the statistical software used. In what follows, I will *not* focus on the R code. Instead, I will focus on the concepts and outputs.
...
```{r message=FALSE}
round(exp(cbind(coef(icu.freq), confint(icu.freq))), 2) # ignore intercept
```

So, for instance, the odds of not making it after hospital discharge increase by about `r round(exp(coef(icu.freq)["cancerYes"]), 0)` times for cancer patients (but see the CI...).
...
The AUC is .85, so the predicted probability of non survival after hospital discharge is much larger for the patients who actually did not make it. So the model is working as intended.
...
For more information on all this, you can start by reading some of the vignettes and blog posts that I mentioned above. Provided you understand what I show below, the rest that you need to learn becomes much more accessible.
...
- Bayesian statistics allows answering questions about parameters *directly*. Some examples will be shown below.
...
For instance, how can we summarize the results of the odds ratio for cancer?

```{r fig.height=6}
plot(icu.bayes, "areas", prob = 0.95, prob_outer = 1, pars = c("cancerYes"), 
     transformations = list(cancerYes = exp))

# Extract samples from posterior:
cancer.MCMC <- as.data.frame(icu.bayes)$cancerYes
OR.cancer   <- exp(cancer.MCMC)                  # odds ratio
median(OR.cancer)                                # median
quantile(OR.cancer, probs = c(.025, .975))       # 95% credible interval
```

What is the probability of the odds ratio being larger than 1?
...
for (i in 1:4)
{
  ...
  for (j in 1:2)
  {
    ...
    polygon(c(age.vec, rev(age.vec)), c(L.025, rev(L.975)), col = adjustcolor(j+1, .10), border = NA)
...
So we learned something by inspecting the posterior predictive distribution. Perhaps we should consider refitting the model, this time adding a constraint for the age effect (would that be theoretically sensible, though?).
