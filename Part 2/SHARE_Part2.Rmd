...
<img src="qrplot.png" width="200" height="200" />
...
Later on, to illustrate how model comparison can be performed under the Bayesian framework, I will also consider the baseline model (no predictors):

$$
\DeclareMathOperator{\logit}{logit}
\begin{align}
\logit(P(\text{died}=Yes))=\alpha.
\end{align}
$$
...
As an example, let's see the generated values of $P(\text{died}=Yes)$, based on the model predictors (age, cancer, admit, uncons):
...
Here is a motto that expresses well one of the main benefits of Bayesian statistics:
...
Note: In this case, the 95% credible interval for the AUC differs markedly from the 95% frequentist confidence interval:

```{r}
roc.out$ci
```

The explanation is that the frequentist CI is based on bootstrap samples (i.e., resamples from the observed data), whereas the Bayesian CI is based on posterior predictive distribution samples. As it happens, these differed quite a lot for these data.
<br><br>
...
We can validate our model by conducting an approximate leave-one-out cross-validation [@vehtari2017]. For example, how much better does our model perform in comparison to the baseline model? Here the idea is that we are looking for the model with better *posterior prediction* ability.

```{r}
icu.bayes.BL     <- update(icu.bayes, died ~ 1)  # Fit baseline model
icu.bayes.BL.loo <- loo(icu.bayes.BL)
compare_models(icu.bayes.BL.loo, icu.bayes.loo)
```
...
icu.bayes.BL.kfold <- rstanarm::kfold(icu.bayes.BL, K = 10)
compare_models(icu.bayes.BL.kfold, icu.bayes.kfold)
```
...
icu.bayes.BL.bf <- update(icu.bayes.BL, diagnostic_file = file.path(tempdir(), "df2.csv"))

icu.bayes.bridge    <- bridge_sampler(icu.bayes.bf)
icu.bayes.BL.bridge <- bridge_sampler(icu.bayes.BL.bf)
```

```{r}
bf(icu.bayes.bridge, icu.bayes.BL.bridge)
```
