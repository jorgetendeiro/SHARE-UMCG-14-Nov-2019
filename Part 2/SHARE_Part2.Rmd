...
Histograms of the estimated probabilities by outcome value (died = No, died = Yes):

```{r message=FALSE}
par(mar = c(4, 6.5, 1, 0), xpd = NA)
layout(matrix(c(1, 2), ncol = 1))

cut <- .3
h1 <- hist(est.prob[died.vec == 0], plot = FALSE)
cutoff.bins <- cut(h1$breaks, c(-Inf, cut, Inf))
plot(h1, col = c("gray", "red")[cutoff.bins], 
     las = 1, freq = FALSE, main = "Died = No", xlab = "")
  arrows (cut+.02, 3, .40, 3, length = 0.08, lwd = 2, col = "red")
text(.5, 3, "1 - Specificity", adj = 0, col = "red")

hist(est.prob[died.vec == 1], freq = FALSE, las = 1, 
     col = c("gray", "red")[cutoff.bins], main = "Died = Yes", xlab = "")
arrows (cut+.02, 1.5, .40, 1.5, length = 0.08, lwd = 2, col = "red")
text(.5, 1.5, "Sensitivity", adj = 0, col = "red")
mtext("Estimated probability", 1, 2)
