...
Say we have data $\mathcal{D}$, which we use to improve our understanding on the possible values of an unknown parameter $\theta$. For instance, $\theta$ can be the incidence rate of a particular medical condition in the population, and $\mathcal{D}$ the scores of a sample of patients on an indicator for that condition.

Bayesian statistics works as follows:

- Select a model by means of a *likelihood function*. The model conceptualizes how theory and data meet: It expresses, for any given value of the model parameter $\theta$, what the probability of *any* observed data is. This is denoted by $p(\mathcal{D}|\theta)$ (read: "probability of $\mathcal{D}$ conditional on a particular $\theta$ value").

- *Before looking at the data*, express your belief about the value of $\theta$ in the population. 'Belief' does not mean 'religious' belief. It means a rational expression of one's knowledge about $\theta$. For instance, suppose the medical condition in our study is very rare: All research thus far suggests that less than 1% of the population is affected by it. This knowledge can (and *should*!) be incorporated in the analysis. The *prior* distribution, denoted $p(\theta)$, does exactly this.
...
Notice that the evidence, $p(\mathcal{D})$, does not depend on $\theta$. It is there just to ensure that the posterior distribution is well defined (among other things that we won't be discussing today). For most purposes we can actually hide it and rewrite Bayes rule like ths:

$$
\text{posterior} \propto \text{prior} \times \text{likelihood},
$$
where $\propto$ means "proportional to". Therefore, the posterior distribution is basically the prior distribution multiplied by the likelihood function.

## Summary

Bayesian statistics allows us to rationally update our knowledge about a phenomenon. All you need is three ingredients:
...
Later on, to illustrate how model comparison can be performed under the Bayesian framework, I will also consider the reduced model from which 'uncons' was removed:
...
(Btw: You should always strive to set up your own priors.)
...
Bayes factors, on the other hand, focus on *prior prediction* qualities of the models under comparison: How well does each model predict the observed data, for the combination of likelihood and priors at hand?
