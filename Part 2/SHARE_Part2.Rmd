...
- a vignette from Aki Vehtari, Jonah Gabry, and Ben Goodrich ([https://avehtari.github.io/modelselection/diabetes.html](https://avehtari.github.io/modelselection/diabetes.html)).

- The plotting tools of the [bayesplot]https://cran.r-project.org/web/packages/bayesplot/index.html) package ([https://mc-stan.org/bayesplot/index.html](https://mc-stan.org/bayesplot/index.html)).

- A blog post by Fabian Dablander ([https://fabiandablander.com/r/Law-of-Practice.html](https://fabiandablander.com/r/Law-of-Practice.html)).
...
plot(est.prob, jitter(died.vec, .2), xlim = c(-.02, 1), ylim = c(-.08, 1.05), 
...
- First of all, the *interpretation* of the results is different. For instance, not that confidence interval $\not=$ credible interval (the latter is *much* more intuitive). 

- Also, we can now answer questions of the type:

    - "What is the probability that the log-odds of predictor X is larger than 1?"
    
    - (Add more examples, depending on how the Bayesian sections below develop).

- Bayesian statistics is very flexible (e.g., relatively easy to adapt model, priors, constraints).

- Also, Bayesian modeling allows really cool model assessment. Below we will briefly highlight the following:
    ...
    - *Posterior predictive distribution*: How does data generated from the estimated model relate to your observed data? This also works for out-of-sample observed data (it is even preferable).
...
icu.bayes.priorPD <- stan_glm(died ~ age + cancer + admit + uncons, 
                      data   = ICU, 
                      family = binomial, 
                      prior_PD = TRUE, seed = 678) # draw from prior predictive dist.
```
...
for (i in 1:4)
{
  hist(priorPD.prob[i, ], xlim = c(0, 1), freq = FALSE, 
       col = "gray", bty = "n", las = 1, 
       xlab = "P(died = Yes)", ylab = "", yaxt = "n", main = "", xaxs = "i", yaxs = "i")
}
```

The model is rather flexible. Some flexibility is good, but too much may be ill devised.

If needed, change the model (e.g., different link function, different priors).
